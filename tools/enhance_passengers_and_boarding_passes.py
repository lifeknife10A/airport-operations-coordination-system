...
# 1. WRITE V1__initial_schema.sql WITH ENHANCED PASSENGERS, BOARDING PASSES & PRINT VIEW
v1_sql = """-- ============================================================
...
print(f"Successfully updated {v1_path} with PNR, names, and Boarding Pass print view!")

# 2. UPDATE V2__seed_data.sql WITH ENHANCED PASSENGER & BOARDING PASS RECORDS
lines = [
    "-- ============================================================",
    "-- AIRPORT OPERATIONS COORDINATION SYSTEM (AOCS)",
    "-- Complete 32-Table Hybrid Seed Dataset (3,200+ Unique Rows)",
    "-- Includes PNR Booking References, Names & Boarding Pass Ticket Data",
    "-- Author: Krishna Solanki & AOCS Engineering Team",
    "-- ============================================================",
    ""
]
...
# 7. AIRCRAFT (100)
lines.append("-- 7. AIRCRAFT (100)")
aircraft_list = []
for i in range(1, 61):
    code = f"VT-A{i:03d}"
    al_id = ((i - 1) % 5) + 1
    aircraft_list.append((i, code, al_id))
...
for aid, reg, al_id in aircraft_list:
    lines.append(f"INSERT INTO aircraft (aircraft_id, registration_number, airline_id) VALUES ({aid}, '{reg}', {al_id}) ON CONFLICT DO NOTHING;")
lines.append("")

# 8. GATES (60)
lines.append("-- 8. GATES (60)")
for i in range(1, 61):
...
lines.append("-- 10. WEATHER_REPORTS (30)")
...
lines.append("-- 11. GATE_ASSIGNMENT_RULES (60)")
ac_types = ["BOEING_737_MAX", "AIRBUS_A320NEO", "BOEING_777_300ER", "AIRBUS_A350_1000", "AIRBUS_A380_800"]
for i in range(1, 61):
    gid = i
    wingspan = 35.8 + (i % 5) * 8.0
    mtow = 79000.0 + (i % 5) * 60000.0
    actype = ac_types[(i - 1) % len(ac_types)]
    lines.append(f"INSERT INTO gate_assignment_rules (rule_id, gate_id, max_wingspan_meters, max_weight_mtow_kg, compatible_aircraft_type) VALUES ({i}, {gid}, {wingspan:.2f}, {mtow:.2f}, '{actype}') ON CONFLICT DO NOTHING;")
lines.append("")

# 12. FLIGHTS (200)
lines.append("-- 12. FLIGHTS (200)")
...
for i in range(1, 121):
    ...
    flights_data.append((i, fnum, st, ftype, orig_ap, dest_ap, al_id, sch_dep, act_dep, sch_arr, act_arr, brd_time, acid, gid, rwid, deptid))
...
lines.append("-- 15. EQUIPMENT_ASSIGNMENTS (200)")
...
# 16. DELAY_LOGS (100)
lines.append("-- 16. DELAY_LOGS (100)")
...
lines.append("-- 18. CARGO_MANIFESTS (150)")
for i in range(1, 151):
...
lines.append("-- 19. BAGGAGE_CAROUSELS (30)")
for i in range(1, 31):
...
# 20. PASSENGERS (300) ENHANCED WITH PNR & NAMES
lines.append("-- 20. PASSENGERS (300)")
pnr_chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"
for i in range(1, 301):
    fn = indian_first[(i - 1) % len(indian_first)] if i <= 180 else intl_first[(i - 181) % len(intl_first)]
    ln = indian_last[(i - 1) % len(indian_last)] if i <= 180 else intl_last[(i - 181) % len(intl_last)]
    ...
    nat = "IND" if i <= 180 else "USA"
    email = f"{fn.lower()}.{ln.lower()}{i}@passenger.com"
    phone = f"+9198{i:08d}" if i <= 180 else f"+1415{i:07d}"
    fid = ((i - 1) % 200) + 1
    lines.append(f"INSERT INTO passengers (passenger_id, first_name, last_name, pnr_code, passport_number, nationality, email, phone_number, flight_id) VALUES ({i}, '{fn}', '{ln}', '{pnr}', '{ppt}', '{nat}', '{email}', '{phone}', {fid}) ON CONFLICT DO NOTHING;")
lines.append("")

# 21. BOARDING_PASSES (300) ENHANCED WITH IATA BCBP PRINTING FIELDS
lines.append("-- 21. BOARDING_PASSES (300)")
...
lines.append("-- 23. BAGGAGE_SCAN_EVENTS (300)")
...
lines.append("-- 24. SECURITY_CHECKPOINTS (20)")
checkpoints_list = [
...
lines.append("-- 25. PASSENGER_CLEARANCE_LOGS (300)")
...
for i in range(1, 301):
    ...
    chkid = ((i - 1) % 20) + 1
    h = (7 + (i % 14)) % 24
    lines.append(f"INSERT INTO passenger_clearance_logs (clearance_id, scan_timestamp, clearance_status, verification_method, passenger_id, boarding_pass_id, checkpoint_id) VALUES ({i}, '2026-07-27 {h:02d}:{(i%60):02d}:00+05:30', '{st}', '{m}', {pid}, {bpid}, {chkid}) ON CONFLICT DO NOTHING;")
lines.append("")

# 26. IMMIGRATION_RECORDS (120)
lines.append("-- 26. IMMIGRATION_RECORDS (120)")
...
lines.append("-- 27. LOUNGE_VISITS (100)")
...
lines.append("-- 28. CUSTOMER_FEEDBACK_LOGS (100)")
...
lines.append("-- 29. AIRLINE_BILLING_INVOICES (24)")
...
lines.append("-- 30. INVOICE_LINE_ITEMS (72)")
...
lines.append("-- 31. NOTIFICATIONS (150)")
notif_titles = [
...
# 32. AUDIT_LOGS (200)
lines.append("-- 32. AUDIT_LOGS (200)")
actions = [
...
print(f"Successfully generated seed data with PNR & Boarding Pass print details in {v2_path}")
