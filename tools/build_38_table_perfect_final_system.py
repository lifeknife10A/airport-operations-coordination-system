...
# 1. WRITE V1__initial_schema.sql (38 PERFECT NORMALIZED POSTGRESQL TABLES + EXPLICIT INDEXES)
v1_sql = """-- ============================================================
...
print(f"Successfully updated {v1_path} with 38 normalized tables and explicit B-Tree Indexes!")

# 2. GENERATE V2__seed_data.sql CLEANLY FROM SCRATCH
lines = [
    "-- ============================================================",
    "-- AIRPORT OPERATIONS COORDINATION SYSTEM (AOCS)",
    "-- Complete 38-Table Master Hybrid Seed Dataset (3,800+ Unique Rows)",
    "-- Includes Turnaround Rotation, Carousel Numbers & All Claude DDL Fixes",
    "-- Author: Krishna Solanki & AOCS Engineering Team",
    "-- ============================================================",
    ""
]
...
# 15. FLIGHTS (200) WITH INBOUND ROTATION LINK
...
for i in range(1, 121):
    ...
    rwid = ((i - 1) % 12) + 1
    deptid = ((i - 1) % 8) + 1
    inbound_id = (i - 1) if (i % 2 == 0 and i > 1) else "NULL"
    
    flights_data.append((i, fnum, st, ftype, orig_ap, dest_ap, al_id, sch_dep, act_dep, sch_arr, act_arr, brd_time, acid, gid, stid, rwid, deptid, inbound_id))
...
for i in range(121, 201):
    ...
    rwid = ((i - 1) % 12) + 1
    deptid = ((i - 1) % 8) + 1
    inbound_id = (i - 1) if (i % 2 == 0 and i > 121) else "NULL"
    
    flights_data.append((i, fnum, st, ftype, orig_ap, dest_ap, al_id, sch_dep, act_dep, sch_arr, act_arr, brd_time, acid, gid, stid, rwid, deptid, inbound_id))

for fid, fnum, st, ftype, orig_ap, dest_ap, al_id, sch_dep, act_dep, sch_arr, act_arr, brd_time, acid, gid, stid, rwid, deptid, inbound_id in flights_data:
    act_dep_val = f"'{act_dep}'" if act_dep != "NULL" else "NULL"
    act_arr_val = f"'{act_arr}'" if act_arr != "NULL" else "NULL"
    lines.append(f"INSERT INTO flights (flight_id, flight_number, flight_status, flight_type, origin_airport_id, destination_airport_id, airline_id, scheduled_departure_time, actual_departure_time, scheduled_arrival_time, actual_arrival_time, boarding_time, aircraft_id, gate_id, stand_id, runway_id, department_id, inbound_flight_id) VALUES ({fid}, '{fnum}', '{st}', '{ftype}', {orig_ap}, {dest_ap}, {al_id}, '{sch_dep}', {act_dep_val}, '{sch_arr}', {act_arr_val}, '{brd_time}', {acid}, {gid}, {stid}, {rwid}, {deptid}, {inbound_id}) ON CONFLICT DO NOTHING;")
lines.append("")
...
# 23. BAGGAGE_CAROUSELS (30) WITH CAROUSEL NUMBER
lines.append("-- 23. BAGGAGE_CAROUSELS (30)")
for i in range(1, 31):
    cnum = f"BELT-{i:02d}"
    term = "T1" if i <= 10 else ("T2" if i <= 20 else "T3")
    fid = (i * 6)
    lines.append(f"INSERT INTO baggage_carousels (carousel_id, carousel_number, terminal, flight_id) VALUES ({i}, '{cnum}', '{term}', {fid}) ON CONFLICT DO NOTHING;")
lines.append("")
...
# 32. IMMIGRATION_RECORDS (120) - WITHOUT REDUNDANT PASSPORT NUMBER
...
for i in range(1, 121):
    ...
    lines.append(f"INSERT INTO immigration_records (immigration_id, visa_type, stamp_number, biometric_facial_matched, clearance_type, passenger_id) VALUES ({i}, '{vtype}', '{snum}', TRUE, '{ctype}', {pid}) ON CONFLICT DO NOTHING;")
lines.append("")
...
# 36. INVOICE_LINE_ITEMS (72) WITH FLIGHT ID MOVEMENT LINK
...
for i in range(1, 73):
    ...
    amt = 2500.0 + (i * 350.0)
    lines.append(f"INSERT INTO invoice_line_items (line_item_id, invoice_id, flight_id, charge_type, amount_usd) VALUES ({i}, {invid}, {fid}, '{ctype}', {amt:.2f}) ON CONFLICT DO NOTHING;")
lines.append("")
