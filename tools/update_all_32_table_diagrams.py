...
# 1. BUILD RELATIONAL SCHEMA XML (32 FULL TABLES)
rel_path = os.path.join(db_dir, "AOCS Relational Schema.drawio.xml")

relational_tables_32 = [
    ...
    {"id": "t_airlines", "title": "AIRLINES", "x": 580, "y": 120, "w": 220, "fields": [("airline_id", "PK"), ("iata_code", ""), ("icao_code", ""), ("airline_name", ""), ("country", "")]},
    {"id": "t_airports", "title": "AIRPORTS", "x": 840, "y": 120, "w": 220, "fields": [("airport_id", "PK"), ("iata_code", ""), ("icao_code", ""), ("airport_name", ""), ("city", "")]},
    {"id": "t_aircraft", "title": "AIRCRAFT", "x": 1100, "y": 120, "w": 220, "fields": [("aircraft_id", "PK"), ("registration_number", ""), ("airline_id", "FK -> AIRLINES.airline_id")]},
    {"id": "t_gates", "title": "GATES", "x": 1360, "y": 120, "w": 190, "fields": [("gate_id", "PK"), ("gate_number", "")]},
    {"id": "t_runways", "title": "RUNWAYS", "x": 1590, "y": 120, "w": 190, "fields": [("runway_id", "PK"), ("runway_code", "")]},

    # ROW 2 (y=450) - USERS & AIRSIDE RULES
    ...
    {"id": "t_gaterules", "title": "GATE_ASSIGNMENT_RULES", "x": 1060, "y": 450, "w": 260, "fields": [("rule_id", "PK"), ("gate_id", "FK -> GATES.gate_id"), ("max_wingspan_meters", ""), ("compatible_aircraft_type", "")]},
    {"id": "t_flights", "title": "FLIGHTS", "x": 1360, "y": 450, "w": 300, "fields": [("flight_id", "PK"), ("flight_number", ""), ("flight_status", ""), ("origin_airport_id", "FK -> AIRPORTS.airport_id"), ("destination_airport_id", "FK -> AIRPORTS.airport_id"), ("airline_id", "FK -> AIRLINES.airline_id"), ("aircraft_id", "FK -> AIRCRAFT.aircraft_id"), ("gate_id", "FK -> GATES.gate_id")]},

    # ROW 3 (y=880) - OPERATIONS, GROUND FLEET & PASSENGERS
    {"id": "t_equipment", "title": "GROUND_EQUIPMENT", "x": 50, "y": 880, "w": 230, "fields": [("equipment_id", "PK"), ("equipment_code", ""), ("equipment_type", ""), ("status", "")]},
    {"id": "t_tasks", "title": "TASKS", "x": 320, "y": 880, "w": 240, "fields": [("task_id", "PK"), ("task_name", ""), ("status", ""), ("flight_id", "FK -> FLIGHTS.flight_id"), ("assigned_user_id", "FK -> USERS.user_id")]},
    ...
    {"id": "t_delays", "title": "DELAY_LOGS", "x": 890, "y": 880, "w": 230, "fields": [("flight_id", "FK -> FLIGHTS.flight_id"), ("delay_seq_no", "PK"), ("delay_minutes", "")]},
    {"id": "t_fuel", "title": "FUEL_LOGS", "x": 1160, "y": 880, "w": 230, "fields": [("fuel_log_id", "PK"), ("fuel_density", ""), ("task_id", "FK -> TASKS.task_id")]},
    {"id": "t_cargo", "title": "CARGO_MANIFESTS", "x": 1430, "y": 880, "w": 240, "fields": [("cargo_id", "PK"), ("container_id", ""), ("fuel_log_id", "FK -> FUEL_LOGS.fuel_log_id")]},
    {"id": "t_baggage", "title": "BAGGAGE_CAROUSELS", "x": 1700, "y": 880, "w": 240, "fields": [("carousel_id", "PK"), ("terminal", ""), ("flight_id", "FK -> FLIGHTS.flight_id")]},
    {"id": "t_passengers", "title": "PASSENGERS", "x": 1970, "y": 880, "w": 240, "fields": [("passenger_id", "PK"), ("passport_number", ""), ("flight_id", "FK -> FLIGHTS.flight_id")]},

    # ROW 4 (y=1260) - DCS BRS, CHECKPOINTS & IMMIGRATION
    {"id": "t_bpasses", "title": "BOARDING_PASSES", "x": 50, "y": 1260, "w": 250, "fields": [("boarding_pass_id", "PK"), ("barcode_data", ""), ("seat_number", ""), ("passenger_id", "FK -> PASSENGERS.passenger_id"), ("flight_id", "FK -> FLIGHTS.flight_id")]},
    {"id": "t_bagtags", "title": "BAG_TAGS", "x": 340, "y": 1260, "w": 240, "fields": [("bag_tag_id", "PK"), ("tag_number", ""), ("weight_kg", ""), ("passenger_id", "FK -> PASSENGERS.passenger_id")]},
    {"id": "t_bagscans", "title": "BAGGAGE_SCAN_EVENTS", "x": 610, "y": 1260, "w": 260, "fields": [("scan_id", "PK"), ("bag_tag_id", "FK -> BAG_TAGS.bag_tag_id"), ("scan_location", ""), ("scan_timestamp", "")]},
    {"id": "t_checkpoints", "title": "SECURITY_CHECKPOINTS", "x": 900, "y": 1260, "w": 250, "fields": [("checkpoint_id", "PK"), ("checkpoint_name", ""), ("checkpoint_type", ""), ("terminal", "")]},
    {"id": "t_clearance", "title": "PASSENGER_CLEARANCE_LOGS", "x": 1180, "y": 1260, "w": 280, "fields": [("clearance_id", "PK"), ("scan_timestamp", ""), ("clearance_status", ""), ("passenger_id", "FK -> PASSENGERS.passenger_id"), ("checkpoint_id", "FK -> SECURITY_CHECKPOINTS.checkpoint_id")]},
    {"id": "t_immigration", "title": "IMMIGRATION_RECORDS", "x": 1490, "y": 1260, "w": 260, "fields": [("immigration_id", "PK"), ("passport_number", ""), ("visa_type", ""), ("stamp_number", ""), ("passenger_id", "FK -> PASSENGERS.passenger_id")]},
    {"id": "t_lounges", "title": "LOUNGE_VISITS", "x": 1780, "y": 1260, "w": 240, "fields": [("visit_id", "PK"), ("lounge_name", ""), ("passenger_id", "FK -> PASSENGERS.passenger_id")]},
    {"id": "t_feedback", "title": "CUSTOMER_FEEDBACK_LOGS", "x": 2050, "y": 1260, "w": 260, "fields": [("feedback_id", "PK"), ("rating", ""), ("category", ""), ("passenger_id", "FK -> PASSENGERS.passenger_id")]},

    # ROW 5 (y=1640) - REVENUE BILLING, AUDIT & NOTIFICATIONS
    {"id": "t_invoices", "title": "AIRLINE_BILLING_INVOICES", "x": 200, "y": 1640, "w": 270, "fields": [("invoice_id", "PK"), ("invoice_number", ""), ("total_amount_usd", ""), ("airline_id", "FK -> AIRLINES.airline_id")]},
    {"id": "t_lineitems", "title": "INVOICE_LINE_ITEMS", "x": 510, "y": 1640, "w": 260, "fields": [("line_item_id", "PK"), ("charge_type", ""), ("amount_usd", ""), ("invoice_id", "FK -> AIRLINE_BILLING_INVOICES.invoice_id")]},
    {"id": "t_notif", "title": "NOTIFICATIONS", "x": 800, "y": 1640, "w": 220, "fields": [("notification_id", "PK"), ("title", ""), ("user_id", "FK -> USERS.user_id")]},
    {"id": "t_audit", "title": "AUDIT_LOGS", "x": 1050, "y": 1640, "w": 230, "fields": [("log_id", "PK"), ("action", ""), ("created_at", ""), ("user_id", "FK -> USERS.user_id")]}
]
...
xml_parts = [
    ...
    '    <mxGraphModel dx="3600" dy="2600" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="3600" pageHeight="2600" math="0" shadow="0">',
    '      <root>',
    '        <mxCell id="0" />',
    '        <mxCell id="1" parent="0" />',
    '',
    '        <!-- HEADER TITLE -->',
    '        <mxCell id="hdr_title" value="&lt;b style=&quot;font-size: 16px;&quot;&gt;AIRPORT OPERATIONS COORDINATION SYSTEM (AOCS)&lt;/b&gt;&lt;br&gt;&lt;span style=&quot;font-size: 13px;&quot;&gt;Complete 32-Table Hybrid Physical Relational Database Schema (Elmasri &amp;amp; Navathe Chapter 7 Mapping)&lt;/span&gt;" style="rounded=0;whiteSpace=wrap;html=1;fontStyle=1;fontSize=14;fillColor=none;strokeColor=default;fontColor=default;" vertex="1" parent="1">',
    '          <mxGeometry x="50" y="30" width="3000" height="50" as="geometry" />',
    '        </mxCell>',
    ''
]
...
for tbl in relational_tables_32:
...
print("Successfully updated AOCS Relational Schema XML with 32 normalized tables!")
